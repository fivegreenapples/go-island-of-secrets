...
// Track the cursor position based on calls to print
// Not perfect but good enough in this case
var cursorX int
...
func TAB(val int) {
	...
	fmt.Print(strings.Repeat(" ", val-cursorX))
	cursorX = val
}
...
func INPUT(r io.Reader) string {
	fmt.Print("?")
	// ToUpper just saves hassle for typing inputs
	return strings.ToUpper(INPUT_DISC(r))
}

func INPUT_NUMERIC(r io.Reader) int {
	return VAL(INPUT(r))
}
...
func ON_GOSUB(val int, subs ...func()) {
	if 0 < val && val <= len(subs) {
		subs[val-1]()
	}
}
